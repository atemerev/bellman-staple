...
$108^\circ$), the polygonal bound $1.2849616\ldots$ (against Ward's
$1.2934740\ldots$) is improved further by an exact line--arc certificate to
$\ell(T)\le1.2826799$, and the critical line--arc path is characterized
exactly: window tangency forces the curvature radius $\sin\beta$ (the width
of $T$), the sines of its arc angles are roots of explicit quartics over
$\Q(\sqrt5)$, and the critical length $1.2826760\ldots$ has the same closed
shape as Zalgaller's strip constant.  Numerically, the line--arc family
continues below the staple range and terminates on the scaled Zalgaller
caliper at $\beta\approx27.6^\circ$, realizing the ``Zalgalloid'' family
Ward speculated about.  The closed-form bracket
$\tfrac{3}{10}\sqrt{25-5\sqrt5}\le\ell(T)$ stands.  Optimality is not
claimed.
\end{abstract}
...
staples, family crossings, the Zalgalloid family) that guided the
constructions but enters no proof.  We emphasize what is \emph{not} claimed: optimality of the staple,
...
scaled caliper at $\beta\approx31.05^\circ$ (pushed to
$\approx27.6^\circ$ by the line--arc family below) and with the zee at
...
numerical; making the junction exact is an inviting next target.
...
The exact contributions of this paper are: a certified upper bound for the
escape length of isosceles triangles on the whole interval
...
beats all four of his comparison families on $[31.50^\circ,42.16^\circ]$; a
finite-certificate method (for polygonal and line--arc data alike) that
replaces the continuum of placements by exact arithmetic; the improved
line--arc bound $\ell(T)\le1.2826799$ at the golden gnomon; and the exact
algebraic characterization of the critical line--arc path there, with its
Zalgaller-form constant.  The numerical contributions --- the curvature law
$r=\sin\beta$, the Zalgalloid family joining the staple to the caliper at
$\beta^{*}\approx27.6^\circ$ --- mark the places where theorems should come
next: a proof of the curvature law via the linear-programming duality it
suggests, an exact treatment of the junction, and certified interval bounds
for the line--arc family.  No optimality is claimed anywhere; the gap in
